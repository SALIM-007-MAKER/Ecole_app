...
function eNoteMention(float $m): string {
...
<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-lg font-bold text-slate-900">Mes notes</h2>
            <?php if ($eleve): ?>
            <p class="text-xs text-slate-400"><?= htmlspecialchars($eleve->classe_nom ?? '', ENT_QUOTES) ?></p>
            <?php endif; ?>
        </div>
    </div>
    <?php if ($eleve && $periodeId): ?>
    <a href="<?= BASE_URL ?>/eleve/bulletin?periode_id=<?= $periodeId ?>" class="btn btn-secondary">
        <i data-lucide="file-badge" class="w-4 h-4"></i>Mon bulletin
    </a>
    <?php endif; ?>
</div>
        ...
        <div class="overflow-x-auto rounded-xl border border-slate-200 bg-white">
            <table class="min-w-full divide-y divide-slate-200 text-sm [&_thead]:bg-slate-50 [&_th]:px-4 [&_th]:py-3 [&_th]:text-left [&_th]:text-[11px] [&_th]:font-bold [&_th]:uppercase [&_th]:tracking-wide [&_th]:text-slate-500 [&_td]:px-4 [&_td]:py-3 [&_td]:align-middle [&_td]:text-slate-700 [&_tbody]:divide-y [&_tbody]:divide-slate-100 [&_tbody_tr:hover]:bg-slate-50 text-sm">
                ...
                <?php foreach ($mat['notes'] as $n):
                    $notePct = (float)($n->bareme ?? 20) > 0
                        ? (float)$n->note / (float)($n->bareme ?? 20)
                        : 0;
                ?>
                <tr>
                    <td class="text-slate-700 font-medium"><?= htmlspecialchars($n->controle_nom ?? 'Contrôle', ENT_QUOTES) ?></td>
                    <td class="text-center font-bold <?= $notePct >= 0.5 ? 'text-emerald-600' : 'text-red-500' ?>">
                    ...
                    <td class="text-xs text-slate-400">
                        <?= !empty($n->date_controle) ? date('d/m/Y', strtotime($n->date_controle)) : '-' ?>
                    </td>
                </tr>
                <?php endforeach; ?>
                </tbody>
