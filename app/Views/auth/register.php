...
<div class="text-center mb-7">
    <div class="w-12 h-12 rounded-2xl bg-violet-600 flex items-center justify-center mx-auto mb-4 shadow-lg shadow-violet-200">
        <i data-lucide="user-plus" class="w-6 h-6 text-white"></i>
    </div>
    <h2 class="text-xl font-bold text-slate-900 mb-1">Créer un compte</h2>
    <p class="text-sm text-slate-500">Votre compte sera activé par un administrateur.</p>
</div>
...
<form method="POST" action="<?= BASE_URL ?>/register" novalidate class="space-y-4">
    <input type="hidden" name="_csrf_token" value="<?= \Core\Session::getCsrfToken() ?>">

    <div class="mb-4">
        <label class="form-label" for="nom">Nom complet <span class="text-red-500">*</span></label>
        <div class="relative flex items-stretch">
            <span class="inline-flex items-center rounded-l-lg border border-r-0 border-slate-300 bg-slate-50 px-3 text-sm text-slate-500"><i data-lucide="user" class="w-4 h-4"></i></span>
            <input type="text"
                   class="form-input pl-10 <?= isset($errors['nom']) ? 'border-red-400' : '' ?>"
                   id="nom" name="nom"
    ...
    <p class="text-center text-sm text-slate-500 pt-1">
        Déjà inscrit ?
        <a href="<?= BASE_URL ?>/login" class="text-violet-600 hover:text-violet-700 font-medium transition-colors">
            Se connecter
        </a>
    </p>
</form>
