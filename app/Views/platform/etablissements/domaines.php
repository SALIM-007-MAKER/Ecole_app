...
<div class="flex items-center justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-slate-500">Domaines associés à <?= htmlspecialchars($etab['nom'], ENT_QUOTES) ?></p>
    </div>
</div>
                ...
                <tr>
                    ...
                    <td class="px-5 py-3 col-actions">
                        <div class="flex items-center gap-2">
                            ...
                            <form method="POST" action="<?= $backUrl ?>/domaines/<?= (int)$d['id'] ?>/verifier" class="inline">
                            ...
                            <?php endif; ?>
                            <?php if ($canEdit): ?>
                            <form method="POST" action="<?= $backUrl ?>/domaines/<?= (int)$d['id'] ?>/toggle" class="inline">
                                <input type="hidden" name="_csrf_token" value="<?= \Core\Session::getCsrfToken() ?>">
                            ...
                            <form method="POST" action="<?= $backUrl ?>/domaines/<?= (int)$d['id'] ?>/supprimer" class="inline" onsubmit="return confirm('Supprimer ce domaine ?');">
                                <input type="hidden" name="_csrf_token" value="<?= \Core\Session::getCsrfToken() ?>">
...
<div class="rounded-xl border border-slate-200 bg-white shadow-sm">
    ...
    <form method="POST" action="<?= $backUrl ?>/domaines" class="p-6 flex items-end gap-3" novalidate>
        <input type="hidden" name="_csrf_token" value="<?= \Core\Session::getCsrfToken() ?>">
        <div class="flex-1">
