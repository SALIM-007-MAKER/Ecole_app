...
<title>Nouvelle évaluation — EduNova</title>
...
<main class="ml-64 p-8 max-w-2xl">
  ...
  <h1 class="text-2xl font-bold text-slate-900 mb-6">Créer une évaluation</h1>
  ...
  <div class="bg-amber-50 border border-amber-200 text-amber-800 rounded-lg p-4 mb-6 text-sm">
    Aucune campagne active. <a href="/v2/rh/evaluations/campagnes/create" class="underline">Créer une campagne</a> d'abord.
  </div>
  <?php endif; ?>

  <form method="POST" action="/v2/rh/evaluations" class="space-y-6">
    <?= \Core\Csrf::field() ?>

    <div class="bg-white border border-slate-100 rounded-xl p-6 shadow-sm space-y-4">
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Campagne *</label>
        <select name="campagne_id" required
                class="w-full border <?= isset($errors['campagne_id']) ? 'border-red-400' : 'border-slate-200' ?> rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-300 outline-none">
      ...
      <div>
        ...
        <select name="employe_id" required
                class="w-full border <?= isset($errors['employe_id']) ? 'border-red-400' : 'border-slate-200' ?> rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-300 outline-none">
      ...
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Évaluateur (responsable)</label>
    ...
    <div class="flex gap-3">
      ...
      <a href="/v2/rh/evaluations" class="px-6 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-lg text-sm hover:bg-slate-50">
        Annuler
      </a>
    </div>
  </form>
