...
<title>Évaluation — <?= e($eval['employe_nom_complet']) ?></title>
  ...
  <div class="flex items-start justify-between mb-8">
    ...
    <div class="flex flex-col gap-2 min-w-[160px]">
      <?php if ($statut === 'brouillon' && $policy->canUpdate($user)): ?>
        <form method="POST" action="/v2/rh/evaluations/<?= (int)$eval['id'] ?>/demarrer-auto-eval">
          <?= \Core\Csrf::field() ?>
      ...
      <?php if ($statut === 'en_evaluation' && $policy->canUpdate($user)): ?>
        <form method="POST" action="/v2/rh/evaluations/<?= (int)$eval['id'] ?>/soumettre">
          <?= \Core\Csrf::field() ?>
      ...
      <?php if ($statut === 'validee' && $policy->canPublish($user)): ?>
        <form method="POST" action="/v2/rh/evaluations/<?= (int)$eval['id'] ?>/publier"
  ...
  <div class="grid grid-cols-3 gap-6">
    ...
    <div class="col-span-2 space-y-6">
      ...
      <div class="bg-white border border-slate-100 rounded-xl p-5 shadow-sm">
        ...
        <form method="POST" action="/v2/rh/evaluations/<?= (int)$eval['id'] ?>/auto-eval" class="mb-6">
          <?= \Core\Csrf::field() ?>
          ...
          <?php endforeach; ?>
          <div class="mt-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">Commentaire général</label>
            <textarea name="commentaire_auto_eval" rows="2"
                      class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-300 outline-none"><?= e($eval['commentaire_auto_eval'] ?? '') ?></textarea>
          </div>
          <button type="submit" class="mt-3 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
        ...
        <form method="POST" action="/v2/rh/evaluations/<?= (int)$eval['id'] ?>/evaluer">
          <?= \Core\Csrf::field() ?>
          ...
          <?php endforeach; ?>
          <div class="mt-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">Commentaire responsable</label>
            <textarea name="commentaire_evaluateur" rows="2"
                      class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-300 outline-none"><?= e($eval['commentaire_evaluateur'] ?? '') ?></textarea>
          </div>
...
<div id="modal-valider" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50">
  <div class="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
    ...
    <form method="POST" action="/v2/rh/evaluations/<?= (int)$eval['id'] ?>/valider">
      <?= \Core\Csrf::field() ?>
      <label class="block text-sm font-medium text-slate-700 mb-1">Commentaire de validation</label>
      <textarea name="commentaire_validation" rows="3"
                class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-300 outline-none mb-4"
...
<div id="modal-plan" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50">
  <div class="bg-white rounded-xl p-6 w-full max-w-lg shadow-xl">
    ...
    <form method="POST" action="/v2/rh/evaluations/<?= (int)$eval['id'] ?>/plan" class="space-y-4">
      <?= \Core\Csrf::field() ?>
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Objectif *</label>
      ...
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Actions à mettre en œuvre</label>
        <textarea name="actions" rows="2"
                  class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-300 outline-none"></textarea>
      </div>
      <div class="grid grid-cols-2 gap-3">
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Ressources nécessaires</label>
