...
<div class="max-w-2xl mx-auto space-y-6">
  <div class="flex items-start gap-4">
    ...
    <h2 class="text-xl font-semibold text-slate-800 pt-2">Ajouter une formation au catalogue</h2>
  </div>
  ...
  <form method="POST" action="<?= BASE_URL ?>/v2/rh/formations/catalogue" class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 space-y-5">
    <?= \Core\Csrf::field() ?>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <input type="text" name="code" value="<?= $e($old['code'] ?? '') ?>"
               ...
               class="form-input <?= isset($err['code']) ? 'is-invalid' : '' ?>">
    ...
    <div>
      ...
      <input type="text" name="titre" value="<?= $e($old['titre'] ?? '') ?>"
             ...
             class="form-input <?= isset($err['titre']) ? 'is-invalid' : '' ?>">
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div>
        <label class="form-label">Max participants / session</label>
    ...
    <div>
      <label class="form-label">Compétences développées</label>
      <?php
        $cats = [];
    ...
    <div>
      ...
      <textarea name="prerequis" rows="2"
                class="form-textarea resize-none"><?= $e($old['prerequis'] ?? '') ?></textarea>
    </div>

    <div>
      <label class="form-label">Objectifs pédagogiques</label>
      <textarea name="objectifs" rows="2"
                class="form-textarea resize-none"><?= $e($old['objectifs'] ?? '') ?></textarea>
    </div>
    ...
    <div class="flex gap-3 pt-2">
      ...
      <a href="<?= BASE_URL ?>/v2/rh/formations/catalogue" class="border border-slate-200 text-slate-600 px-6 py-2 rounded-lg text-sm hover:bg-slate-50 transition">
        Annuler
      </a>
    </div>
  </form>
