...
<main class="ml-64 p-8 max-w-5xl">
    ...
    <div class="flex items-start justify-between mb-6">
      <div>
        <div class="flex items-center gap-3">
          <h1 class="text-xl font-bold text-slate-900"><?= e($dept['nom']) ?></h1>
      ...
      <div class="flex gap-2">
        <?php if ($canUpdate && !$archived): ?>
          <a href="/v2/rh/organisation/departements/<?= (int)$dept['id'] ?>/edit"
             class="px-4 py-2 bg-violet-600 text-white text-sm rounded-lg hover:bg-violet-700">Modifier</a>
        <?php endif; ?>
        <?php if ($canArchive && !$archived): ?>
          <form method="POST" action="/v2/rh/organisation/departements/<?= (int)$dept['id'] ?>/archive"
            ...
            <?php \Core\Csrf::field(); ?>
            <button type="submit" class="px-4 py-2 bg-amber-50 text-amber-700 border border-amber-200 text-sm rounded-lg hover:bg-amber-100">Archiver</button>
          </form>
        <?php elseif ($canArchive && $archived): ?>
          <form method="POST" action="/v2/rh/organisation/departements/<?= (int)$dept['id'] ?>/restore">
            <?php \Core\Csrf::field(); ?>
            <button type="submit" class="px-4 py-2 bg-emerald-50 text-emerald-700 border border-emerald-200 text-sm rounded-lg hover:bg-emerald-100">Restaurer</button>
          </form>
        <?php endif; ?>
      </div>
    </div>
  ...
  <div class="bg-white rounded-xl border border-slate-200 p-6 mb-6">
    <div class="flex items-center justify-between mb-4">
      <h2 class="font-semibold text-slate-800">Services rattachés (<?= count($services) ?>)</h2>
      <?php if ($canUpdate && !$archived): ?>
        <button onclick="document.getElementById('form-new-service').classList.toggle('hidden')"
                class="text-sm text-violet-600 hover:underline">+ Ajouter un service</button>
      <?php endif; ?>
    </div>
    ...
    <?php if ($canUpdate && !$archived): ?>
      <form id="form-new-service" method="POST"
            action="/v2/rh/organisation/departements/<?= (int)$dept['id'] ?>/services"
        ...
        <div class="grid grid-cols-2 gap-4 mb-3">
          ...
          <div>
            ...
            <input type="text" name="code" required placeholder="EX_CODE" pattern="[A-Z0-9_]{2,30}"
                   class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-300 font-mono uppercase">
          </div>
        </div>
        <div class="mb-3">
        ...
        <div class="flex gap-2 justify-end">
          <button type="button" onclick="document.getElementById('form-new-service').classList.add('hidden')"
                  class="px-3 py-1.5 text-sm bg-slate-200 text-slate-600 rounded-lg hover:bg-slate-300">Annuler</button>
          <button type="submit" class="px-3 py-1.5 text-sm bg-violet-600 text-white rounded-lg hover:bg-violet-700">Créer le service</button>
        </div>
        ...
        <?php foreach ($services as $svc): ?>
          <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <?php if ($canArchive && $svc['deleted_at'] === null): ?>
              <form method="POST"
                    action="/v2/rh/organisation/departements/<?= (int)$dept['id'] ?>/services/<?= (int)$svc['id'] ?>/archive"
  ...
  <?php if (!empty($historique)): ?>
    <div class="bg-white rounded-xl border border-slate-200 p-6">
      <h2 class="font-semibold text-slate-800 mb-4">Historique des modifications</h2>
      <div class="space-y-2">
