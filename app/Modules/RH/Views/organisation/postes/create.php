...
?>
  ...
  <div class="flex items-start gap-4 mb-8">
    <div class="w-11 h-11 rounded-xl bg-violet-100 flex items-center justify-center flex-shrink-0">
      <i data-lucide="briefcase" class="w-5 h-5 text-violet-600"></i>
    </div>
    <h1 class="text-2xl font-bold text-slate-900 pt-2">Nouveau poste</h1>
  </div>
  ...
  <form method="POST" action="<?= BASE_URL ?>/v2/rh/organisation/postes" class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5">
    <?php \Core\Csrf::field(); ?>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <select name="categorie" required
                class="form-select <?= isset($errors['categorie']) ? 'is-invalid' : '' ?>">
    ...
    <div>
      ...
      <textarea name="description" rows="3" placeholder="Missions, responsabilités, prérequis..."
                class="form-textarea resize-none"><?= old('description', $old) ?></textarea>
    </div>
