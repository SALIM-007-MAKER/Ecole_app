...
<div class="space-y-6">
  ...
  <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
    <form method="GET" class="flex gap-3 items-end">
      ...
      <a href="/v2/rh/formations/competences" class="border border-slate-200 text-slate-600 px-4 py-2 rounded-lg text-sm hover:bg-slate-50">Réinitialiser</a>
    </form>
  </div>
...
<div id="modal-comp" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
  <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
    ...
    <form method="POST" action="/v2/rh/formations/competences">
      <?= \Core\Csrf::field() ?>
      <input type="hidden" name="employe_id" value="<?= (int)$employeId ?>">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Compétence <span class="text-red-500">*</span></label>
          <select name="competence_id" required class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-400 outline-none">
        ...
        <div>
          ...
          <select name="niveau" required class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-400 outline-none">
