...
<div class="max-w-2xl mx-auto space-y-6">
  <div class="flex items-center gap-3">
    <a href="/v2/rh/documents" class="text-slate-400 hover:text-slate-600">
  ...
  <form method="POST" action="/v2/rh/documents" class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 space-y-5">
    <?= \Core\Csrf::field() ?>

    <div class="grid grid-cols-2 gap-4">
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Type <span class="text-red-500">*</span></label>
        <select name="type" required class="w-full border <?= isset($err['type']) ? 'border-red-400' : 'border-slate-200' ?> rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-400 outline-none">
      ...
      <div>
        ...
        <select name="employe_id" required class="w-full border <?= isset($err['employe_id']) ? 'border-red-400' : 'border-slate-200' ?> rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-400 outline-none">
          <option value="">Sélectionner…</option>
    ...
    <div class="grid grid-cols-3 gap-4">
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Date d'émission <span class="text-red-500">*</span></label>
      ...
      <div>
        ...
        <input type="date" name="date_expiration" value="<?= $e($old['date_expiration'] ?? '') ?>"
               class="w-full border <?= isset($err['date_expiration']) ? 'border-red-400' : 'border-slate-200' ?> rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-400 outline-none">
        <?php if ($err['date_expiration'] ?? ''): ?><p class="text-xs text-red-500 mt-1"><?= $e($err['date_expiration']) ?></p><?php endif; ?>
      </div>
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Alerte (jours avant)</label>
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Émetteur / Autorité</label>
    ...
    <div>
      ...
      <input type="text" name="reference_externe" value="<?= $e($old['reference_externe'] ?? '') ?>"
             ...
             class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-violet-400 outline-none">
    ...
    <div class="flex gap-3 pt-2">
      ...
      <a href="/v2/rh/documents" class="border border-slate-200 text-slate-600 px-6 py-2 rounded-lg text-sm hover:bg-slate-50 transition">
        Annuler
      </a>
    </div>
  </form>
