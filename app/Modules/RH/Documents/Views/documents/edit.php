...
<div class="max-w-2xl mx-auto space-y-6">
  <div class="flex items-center gap-3">
    <a href="/v2/rh/documents/<?= (int)$doc['id'] ?>" class="text-slate-400 hover:text-slate-600">
  ...
  <form method="POST" action="/v2/rh/documents/<?= (int)$doc['id'] ?>" class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 space-y-5">
    <?= \Core\Csrf::field() ?>
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Émetteur</label>
    ...
    <div>
      ...
      <textarea name="notes" rows="3"
                class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-400 outline-none resize-none"><?= $e($old['notes'] ?? $doc['notes'] ?? '') ?></textarea>
    </div>

    <div class="border-t border-slate-100 pt-4">
      <label class="block text-sm font-medium text-slate-700 mb-1">Motif de la nouvelle version <span class="text-red-500">*</span></label>
    ...
    <div class="flex gap-3 pt-2">
      ...
      <a href="/v2/rh/documents/<?= (int)$doc['id'] ?>" class="border border-slate-200 text-slate-600 px-6 py-2 rounded-lg text-sm hover:bg-slate-50 transition">
        Annuler
      </a>
    </div>
  </form>
