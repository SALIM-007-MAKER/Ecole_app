<?php /** @var array $enfants, array $notes */ ?>
<div class="space-y-4">
  <?php if (empty($notes)): ?>
    ...
    <table class="min-w-full divide-y divide-slate-200 text-sm">
      ...
      <tbody class="divide-y divide-slate-100">
        ...
        <tr class="hover:bg-slate-50">
          <td class="px-4 py-3 text-slate-600"><?= htmlspecialchars($n['enfant_nom'] ?? '') ?></td>
          <td class="px-4 py-3 font-medium text-slate-800"><?= htmlspecialchars($n['matiere_nom'] ?? '') ?></td>
          <td class="px-4 py-3"><span class="font-bold text-emerald-700"><?= number_format((float)($n['valeur'] ?? 0), 2) ?></span>/<?= (int)($n['bareme'] ?? 20) ?></td>
