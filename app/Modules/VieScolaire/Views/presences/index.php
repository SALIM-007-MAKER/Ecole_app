...
function epPres(array $p, string $k): bool { return in_array($k, $p, true); }

$statutColors = ['brouillon' => 'amber', 'valide' => 'emerald'];
$statutLabels = ['brouillon' => 'Brouillon', 'valide' => 'Validé'];
$typeLabels   = ['journalier' => 'Journalier', 'seance' => 'Par séance'];
?>
...
<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">Gestion des sessions d'appel — Module Vie Scolaire V2</p>
    </div>
    <div class="flex items-center gap-2 flex-wrap">
        <?php if (epPres($perms, 'attendance.session.view')): ?>
        <a href="<?= BASE_URL ?>/v2/vie-scolaire/presences/statistiques"
        ...
        <?php endif; ?>
        <?php if (epPres($perms, 'attendance.session.create')): ?>
        <a href="<?= BASE_URL ?>/v2/vie-scolaire/presences/create"
           class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-violet-600 text-white text-sm font-medium hover:bg-violet-700 transition-colors">
            <i data-lucide="plus" class="w-4 h-4"></i>Nouvel appel
        </a>
        <?php endif; ?>
    </div>
</div>
...
<div class="bg-white border border-slate-200 rounded-xl shadow-sm p-4 mb-6">
    <form method="GET" class="flex flex-wrap gap-3 items-end">
        ...
        <a href="<?= BASE_URL ?>/v2/vie-scolaire/presences" class="text-sm text-slate-500 hover:text-slate-700 self-end py-1.5">Réinitialiser</a>
    </form>
</div>
...
<div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
        <span class="text-sm font-medium text-slate-700">
            <?= number_format($result['total']) ?> session<?= $result['total'] > 1 ? 's' : '' ?>
        </span>
    </div>
    ...
    <div class="py-16 text-center text-slate-400">
        ...
        <p class="text-sm">Aucune session d'appel trouvée.</p>
    </div>
    <?php else: ?>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    ...
                    <th class="text-left px-4 py-3 font-medium text-slate-600">Matière / Type</th>
                    <th class="text-left px-4 py-3 font-medium text-slate-600">Enseignant</th>
                    <th class="text-center px-4 py-3 font-medium text-slate-600">Pointés</th>
            ...
            <?php foreach ($result['data'] as $row):
                $sColor = $statutColors[$row['statut']] ?? 'slate';
                $sLabel = $statutLabels[$row['statut']] ?? $row['statut'];
                $tLabel = $typeLabels[$row['type_appel']]  ?? $row['type_appel'];
            ?>
            <tr class="hover:bg-slate-50 transition-colors">
                <td class="px-4 py-3 font-medium text-slate-900 whitespace-nowrap">
                    <?= date('d/m/Y', strtotime($row['date_appel'])) ?>
                    <?php if (!empty($row['heure_debut'])): ?>
                    <span class="text-xs text-slate-400 ml-1"><?= htmlspecialchars($row['heure_debut'], ENT_QUOTES) ?></span>
                    <?php endif; ?>
                </td>
                <td class="px-4 py-3 text-slate-700"><?= htmlspecialchars($row['classe_nom'], ENT_QUOTES) ?></td>
                <td class="px-4 py-3 text-slate-600">
                    <?php if (!empty($row['matiere_nom'])): ?>
                        <?= htmlspecialchars($row['matiere_nom'], ENT_QUOTES) ?>
                    <?php else: ?>
                        <span class="text-slate-400"><?= htmlspecialchars($tLabel, ENT_QUOTES) ?></span>
                    <?php endif; ?>
                </td>
                <td class="px-4 py-3 text-slate-600"><?= htmlspecialchars($row['enseignant_nom'] ?? '', ENT_QUOTES) ?></td>
                ...
                <td class="px-4 py-3">
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-<?= $sColor ?>-100 text-<?= $sColor ?>-700">
                        <?= htmlspecialchars($sLabel, ENT_QUOTES) ?>
                    </span>
                </td>
                <td class="px-4 py-3">
                    <a href="<?= BASE_URL ?>/v2/vie-scolaire/presences/<?= $row['id'] ?>"
                       class="inline-flex items-center gap-1 text-xs text-violet-600 hover:text-violet-800 font-medium">
                        <i data-lucide="eye" class="w-3.5 h-3.5"></i>Appel
                    </a>
                </td>
            </tr>
            <?php endforeach; ?>
    ...
    <div class="px-5 py-4 border-t border-slate-100 flex items-center justify-between text-sm">
        <span class="text-slate-500">Page <?= $result['page'] ?> / <?= $result['last_page'] ?></span>
